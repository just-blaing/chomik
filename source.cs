...
private void AnimationTimer_Tick(object? sender, EventArgs e)
{
    ...
    {
        Debug.WriteLine($"Error in CustomMessageBox AnimationTimer_Tick: {ex.Message}");
        animationTimer?.Stop();
    }
}
...
public HamsterViewer()
{
    ...
    textMenuItem = new ToolStripMenuItem("chomik v 1.0");
...
private void PopulateUninterruptibleAnimations()
{
    ...
    uninterruptibleAnimations.Add("AnimCharacterMoving");
...
{
    bool previousMusicState = isMusicPlaying;
    bool checkOk = false;
    try
    {
        ...
        if (currentSession != null)
        {
            ...
            isMusicPlaying = playbackInfo.PlaybackStatus == GlobalSystemMediaTransportControlsSessionPlaybackStatus.Playing;
            checkOk = true;
        }
        else
        {
    ...
    if (checkOk && isMusicPlaying != previousMusicState)
    {
        if (isMusicPlaying)
        {
            if (currentAnimationName != currentMusicStart && currentAnimationName != currentMusicLoop)
            {
                if (!uninterruptibleAnimations.Contains(currentAnimationName))
                {
...
private void LoadInitialAnimation()
{
    ...
    {
        if (currentAnimationName == "AnimMainIdle" || !loadedAnimations.ContainsKey(currentAnimationName))
        {
            LoadAnimation("AnimMainIdle");
            currentAnimationName = "AnimMainIdle";
        }
...
private void HandleAnimationFinish()
{
    ...
    TimeSpan elapsedSinceLastKeyPress = DateTime.Now - lastKeyPressTime;
    bool isUserCurrentlyTypingLetters = elapsedSinceLastKeyPress.TotalMilliseconds < typingDurationThresholdMs;
    if (previousAnimationName == "AnimTypingStop")
    {
        isTypingAnimationActive = false;
        typingSessionStartTime = DateTime.MinValue;
         if (!isMusicPlaying && !isDragging)
         ...
         {
              nextAnimation = loadedAnimations.ContainsKey(currentMusicLoop) ? currentMusicLoop : "AnimMainIdle";
         }
         else if (isDragging)
         {
              nextAnimation = loadedAnimations.ContainsKey("AnimCharacterMoving") ? "AnimCharacterMoving" : "AnimMainIdle";
         }
    }
    else if (previousAnimationName == "AnimCharacterMoveFinish")
    {
        isDragging = false;
         if (!isMusicPlaying && !isTypingAnimationActive)
         {
              if (random.Next(100) < 20 && oneOffRandomIdleAnimations.Any(a => loadedAnimations.ContainsKey(a)))
              {
                  var availableRandomIdles = oneOffRandomIdleAnimations.Where(a => loadedAnimations.ContainsKey(a)).ToList();
         ...
         else if (isTypingAnimationActive)
         {
             nextAnimation = loadedAnimations.ContainsKey("AnimTyping") ? "AnimTyping" : "AnimMainIdle";
         }
    }
    ...
    {
        if (previousAnimationName == currentMusicStart)
        {
            nextAnimation = loadedAnimations.ContainsKey(currentMusicLoop) ? currentMusicLoop : "AnimMainIdle";
        }
        else if (previousAnimationName == currentMusicLoop)
        {
            nextAnimation = currentMusicLoop;
        }
        else
        {
            nextAnimation = loadedAnimations.ContainsKey(currentMusicStart) ? currentMusicStart : (loadedAnimations.ContainsKey(currentMusicLoop) ? currentMusicLoop : "AnimMainIdle");
        }
    }
    else if (isUserCurrentlyTypingLetters)
    {
        if (previousAnimationName == "AnimTypingStop")
        {
            nextAnimation = loadedAnimations.ContainsKey("AnimTypingStart") ? "AnimTypingStart" : (loadedAnimations.ContainsKey("AnimTyping") ? "AnimTyping" : "AnimMainIdle");
        }
        else if (previousAnimationName == "AnimTypingStart")
        {
            nextAnimation = loadedAnimations.ContainsKey("AnimTyping") ? "AnimTyping" : "AnimMainIdle";
        }
        else if (previousAnimationName == "AnimTyping")
        {
    ...
    else if (isDragging)
    {
        ...
        {
             nextAnimation = loadedAnimations.ContainsKey("AnimCharacterMoveStart") ? "AnimCharacterMoveStart" : (loadedAnimations.ContainsKey("AnimCharacterMoving") ? "AnimCharacterMoving" : "AnimMainIdle");
        }
    }
    else if (isRandomIdleSequence && (previousAnimationName == currentIdleStart || previousAnimationName == currentIdleLoop))
    {
        if (previousAnimationName == currentIdleStart)
        {
            nextAnimation = loadedAnimations.ContainsKey(currentIdleLoop) ? currentIdleLoop : (loadedAnimations.ContainsKey(currentIdleFinish) ? currentIdleFinish : "AnimMainIdle");
            if (loadedAnimations.ContainsKey(currentIdleLoop)) idleLoopCounter = 1;
        }
        else if (previousAnimationName == currentIdleLoop && idleLoopCounter < maxIdleLoops)
        {
        ...
        {
            nextAnimation = loadedAnimations.ContainsKey(currentIdleFinish) ? currentIdleFinish : "AnimMainIdle";
        }
    }
        ...
        else
        {
            if (random.Next(100) < 10)
            {
                ...
                if (availableStartAnims.Count > 0)
                {
                    ...
                    int startNumber = int.Parse(currentIdleStart.Replace("AnimIdleStart", ""));
...
private void Form_MouseDown(object? sender, MouseEventArgs e)
{
    if (e.Button == MouseButtons.Left)
    {
        mouseOffset = new Point(-e.X, -e.Y);
        isMouseDown = true;
        if (!isMusicPlaying && !isTypingAnimationActive && !isDragging)
        {
...
private void Form_MouseMove(object? sender, MouseEventArgs e)
{
    if (isMouseDown)
    {
        ...
        mousePos.Offset(mouseOffset.X, mouseOffset.Y);
        Location = mousePos;
        if (isDragging && currentAnimationName != "AnimCharacterMoveStart" && currentAnimationName != "AnimCharacterMoving")
        {
            if (loadedAnimations.ContainsKey("AnimCharacterMoving"))
            {
                LoadAnimation("AnimCharacterMoving");
                currentAnimationName = "AnimCharacterMoving";
            }
...
private void Form_MouseUp(object? sender, MouseEventArgs e)
{
    if (e.Button == MouseButtons.Left)
    {
        ...
        if (isDragging)
        {
            ...
            {
                if (currentAnimationName != "AnimCharacterMoveFinish")
                {
